...
TRACKER_PATH = "app/resources/tracker/botsort.yaml"
    ...
    while True:
        ...
        for vehicle in parked_vehicles['list']:
            ...
            if slot_name == "" or slot_name is None:
                ...
                if empty_slot_count[license_plate] % 3 == 0: # cảnh báo được lặp lại sau mỗi chu kỳ
...
def start_tracking_car():
    ...
    shared_give_way = manager.Value('b', False)  # Shared boolean cho give_way

    globals.bbox_by_cam = shared_bbox_by_cam
    globals.global_id_license_plate_map = shared_license_map
    globals.give_way_shared = shared_give_way  # Gán shared variable vào globals
    globals.give_way = shared_give_way.value  # Giữ reference local cho compatibility

    camera_configs = []
    # Thiết lập camera_configs và tải tọa độ từ cloud server
    for i, cam_source in enumerate(VIDEO_SOURCES):
        ...
        if cam is not None:
            ...
            write_yaml_file(f"{SLOT_COORDS_PATH}{i}.yml", slot_coordinates_data)
            write_yaml_file(f"{REID_COORDS_PATH}{i}.yml", reid_coordinates_data)
